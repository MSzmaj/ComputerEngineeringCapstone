...
int getBMP180Readings (long &rawTemperature, long &rawPressure, int &timer) {
    ...
    if (timer == temperatureMeasurementDelay && rawTemperature == 0) {
        rawTemperature = getTemperature();
        startPressureMeasurement();
    }
    if (timer == temperatureMeasurementDelay + getUncompensatedPressureDelay() && rawPressure == 0) {
...
long getPressure () {
    ...
    //Serial.print("UP: "); Serial.println(uncompensatedPressure);
    B6 = (B5 - 4000);
#if verbose
...
String compiledBMP180Data (long temperature, long pressure) {
    ...
    returnString += String(temperature/10.0f);
    returnString += ",P";
    returnString += String(pressure/1000.0f);
    return returnString;
}
